...
import { generateStorageTextureBindingDeclare } from './storage_texture_binding_declare_helper.js';
import { generateTextureBindingDeclare } from './texture_binding_declare_helper.js';
...
export function generateWgslBindingDeclare(group: number, binding: GPUBindGroupLayoutEntry) {
  ...
  if (binding.buffer !== undefined) {
    ...
    declare = generateStorageTextureBindingDeclare(binding.storageTexture);
  } else if (binding.texture !== undefined) {
    declare = generateTextureBindingDeclare(binding.texture);
  } else {
    unreachable();
  }
